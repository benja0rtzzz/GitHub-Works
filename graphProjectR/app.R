...
server <- function(input, output) {
    ...
    output$distPlot <- renderPlot({
      ...
      getConnections <- function(receivedInput, connectionsVector){
        m <- str_extract_all(receivedInput, regex("[A-Z]"))
        return(m)
      }
      
      getEdges <- function(receivedInput, edgesVector){
        ...
        expr = {
          if(validateString(boxInput, length(boxInput))){
            for(i in 1:length(boxInput)){
              connections <- append(connections, getConnections(boxInput[i], connections))
              edges <- append(edges, getEdges(boxInput[i], edges))
            }
    ...
    output$name <- renderText({
      paste("Created by Benjamin Ortiz")
    })
}
